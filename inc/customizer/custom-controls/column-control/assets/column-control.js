jQuery(document).ready(function ($) {
    $('.customize-control-online-magazine-column').each(function () {
        var columnSelector = $(this).find('.online-magazine-column-selector')[0];
        ...
        online_magazine_update_nouislider(columnSelector, $th);
    });

    $('.online-magazine-add-col').on('click', function () {
        $th = $(this);
        var columnSelectorAdd = $(this).closest('.customize-control-online-magazine-column').find('.online-magazine-column-selector')[0];
        var count = $(this).closest('.customize-control-online-magazine-column').find('input').val().split(',').length + 1;

        online_magazine_set_uislider(columnSelectorAdd, count);

        if (count != 6) {
            online_magazine_update_nouislider(columnSelectorAdd, $th.closest('.customize-control-online-magazine-column'));
        }

        return false;
    });

    $('.online-magazine-remove-col').on('click', function () {
        $th = $(this);
        var columnSelectorRemove = $(this).closest('.customize-control-online-magazine-column').find('.online-magazine-column-selector')[0];
        var count = $(this).closest('.customize-control-online-magazine-column').find('input').val().split(',').length - 1;

        online_magazine_set_uislider(columnSelectorRemove, count);

        if (count != 0) {
            online_magazine_update_nouislider(columnSelectorRemove, $th.closest('.customize-control-online-magazine-column'));
        }

        return false;
    });

    $('.online-magazine-reset-col').on('click', function () {
        var columnSelectorReset = $(this).closest('.customize-control-online-magazine-column').find('.online-magazine-column-selector')[0];
        var count = $(this).closest('.customize-control-online-magazine-column').find('input').val().split(',').length;
        if (count == 1) {
    ...
    function online_magazine_update_nouislider(columnSelector, $elements) {
    ...
    function online_magazine_set_uislider(columnSelector, count) {
        if (count == 0) {
