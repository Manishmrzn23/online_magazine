...
final class Icon_Picker_Fontpack {
	...
	protected function __construct() {
		...
		$this->messages = array(
			'no_config'    => __( 'Icon Picker: %1$s was not found in %2$s.', 'square-plus' ),
			'config_error' => __( 'Icon Picker: %s contains an error or more.', 'square-plus' ),
			'invalid'      => __( 'Icon Picker: %1$s is not set or invalid in %2$s.', 'square-plus' ),
			'duplicate'    => __( 'Icon Picker: %1$s is already registered. Please check your font pack config file: %2$s.', 'square-plus' ),
		);
	...
	protected function get_pack_data( DirectoryIterator $pack_dir ) {
		...
		$pack_data = array(
			'id'             => "pack-{$config['name']}",
			'name'           => sprintf( __( 'Pack: %s', 'square-plus' ), $config['name'] ),
