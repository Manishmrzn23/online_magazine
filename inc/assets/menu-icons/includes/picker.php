...
final class Menu_Icons_Picker {
	...
	protected static function _get_menu_item_fields( $meta ) {
		$fields = array_merge(
			array(
				array(
					'id'    => 'type',
					'label' => __( 'Type', 'square-plus' ),
					'value' => $meta['type'],
				),
				array(
					'id'    => 'icon',
					'label' => __( 'Icon', 'square-plus' ),
					'value' => $meta['icon'],
				),
			),
		...
		?>
			<div class="field-icon description-wide menu-icons-wrap" data-id="<?php echo json_encode( $item->ID ); ?>">
				...
				<p class="description submitbox">
					<label><?php esc_html_e( 'Icon:', 'square-plus' ) ?></label>
					<?php printf( '<a class="_select">%s</a>', esc_html__( 'Select', 'square-plus' ) ); ?>
					<?php printf( '<a class="_remove submitdelete hidden">%s</a>', esc_html__( 'Remove', 'square-plus' ) ); ?>
				</p>
	...
	public static function _columns( $columns ) {
		$columns['icon'] = __( 'Icon', 'square-plus' );
