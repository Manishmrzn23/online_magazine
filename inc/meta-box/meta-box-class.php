 ...
 */
if (!class_exists( 'Online_Magazine_Meta_Box' )) {
     ...
     * @todo Nothing.
     */
    class Online_Magazine_Meta_Box {
        ...
        public function load_scripts_styles() {
            ...
            if (in_array($typenow, $this->_meta_box['pages']) && $this->is_edit_page()) {
                ...
                wp_enqueue_style( 'online-magazine-meta-box', $plugin_path . '/assets/meta-box.css' );

                // Enqueue Meta Box Scripts
                wp_enqueue_script( 'online-magazine-meta-box', $plugin_path . '/assets/meta-box.js', array( 'jquery' ), null, true);
        ...
        public function show_field_repeater($field, $meta) {
            ...
            echo "<div class='online-magazine-repeater" . $class . "' id='{$field['id']}'>";
            ...
            if (count($meta) > 0 && is_array($meta)) {
                foreach ($meta as $me) {
                    ...
                    if (in_array($field['fields'][0]['type'], array( 'image', 'file' )))
                        $mmm = $c + 1;
                    echo '<div class="online-magazine-repater-block">';
                    echo '<h6>';
                    if ($field['sortable']) {
                        echo '<span class="re-control online-magazine-re-sort-handle"><i class="icofont-drag"></i></span>';
                    }
                    echo $mmm;
                    echo '<span class="re-control online-magazine-re-toggle"><i class="icofont-simple-down"></i></span>';
                    echo '</h6>';
                    ...
                    echo '<span class="re-control online-magazine-re-remove" id="remove-' . $field['id'] . '"><i class="icofont-close-circled"></i></span>';
                    echo '</div>';
                    echo '</div>';
                    $c = $c + 1;
                }
        ...
        public function show_field_gallery($field, $meta) {
            ...
            echo '</ul>';
            echo '<input type="hidden" name="' . $field['id'] . '" value="' . $meta . '" /><a href="#" class="button ht-meta-box-gallery-button">Add Images</a>';
        ...
        public function show_field_text($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input type='text' class='online-magazine-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' " . ( isset($field['style']) ? "style='{$field['style']}'" : '' ) . "/>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_number($field, $meta) {
            ...
            echo "<input type='number' class='online-magazine-number" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' " . $step . $min . $max . ( isset($field['style']) ? "style='{$field['style']}'" : '' ) . "/>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_hidden($field, $meta) {
            echo "<input type='hidden' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . "class='online-magazine-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}'/>";
        }
        ...
        public function show_field_textarea($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<textarea class='online-magazine-textarea large-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " cols='60' rows='10'>{$meta}</textarea>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_select($field, $meta) {
            ...
            $this->show_field_begin($field, $meta);
            echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . ">";
        ...
        public function show_field_widget_list($field, $meta) {
            ...
            if ($wp_registered_sidebars) {
                echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-widget-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . ">";
                echo "<option value='none'" . selected(in_array( 'none', $meta), true, false) . ">" . esc_html__( '-- Choose Widget --', 'online-magazine' ) . "</option>";
        ...
        public function show_field_radio($field, $meta) {
            ...
            $this->show_field_begin($field, $meta);
            foreach ($field['options'] as $key => $value) {
                echo "<input type='radio' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-radio" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' value='{$key}'" . checked(in_array($key, $meta), true, false) . " /> <span class='online-magazine-radio-label'>{$value}</span>";
            }
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_image_radio($field, $meta) {
            ...
            foreach ($field['options'] as $key => $value) {
                ...
                echo "<img src='{$value}'>";
                echo "<input type='radio' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-radio' name='{$field['id']}' value='{$key}'" . checked(in_array($key, $meta), true, false) . " />";
                echo "<span></span>";
                echo "</label>";
            }
            $this->show_field_end($field, $meta);
        ...
        public function show_field_wysiwyg($field, $meta, $in_repeater = false) {
            ...
            if ($in_repeater)
                echo "<textarea class='online-magazine-wysiwyg theEditor large-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>{$meta}</textarea>";
            else {
                $settings = ( isset($field['settings']) && is_array($field['settings']) ? $field['settings'] : array() );
                $settings['editor_class'] = 'online-magazine-wysiwyg' . ( isset($field['class']) ? ' ' . $field['class'] : '' );
        ...
        public function show_field_background($field, $meta) {
            ...
            $std = isset($field['std']) ? $field['std'] : array( 'id' => '', 'url' => '', 'repeat' => 'no-repeat', 'size' => 'auto', 'position' => 'center center', 'attachment' => 'scroll', 'color' => '' );
            ...
            echo "<input class='online-magazine-color-iris' type='text' name='{$name}[color]' value='{$value['color']}' size='8' />";
            echo '</div>';
        ...
        public function show_field_color($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input class='online-magazine-color-iris" . (isset($field['class']) ? " {$field['class']}" : "") . "' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' />";
        ...
        public function show_field_alpha_color($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input data-alpha-enabled='true' class='online-magazine-color-iris" . (isset($field['class']) ? " {$field['class']}" : "") . "' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' />";
        ...
        public function show_field_checkbox_list($field, $meta) {
            ...
            foreach ($field['options'] as $key => $value) {
                ...
                echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . "  class='online-magazine-checkbox_list" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='{$key}'" . checked(in_array($key, $meta), true, false) . " /> {$value}";
                echo "</div>";
            }
        ...
        public function show_field_date($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input type='text'  " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-date" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='{$meta}' size='30' />";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_time($field, $meta) {
            ...
            echo "<input type='text'  " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-time" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' data-ampm='{$ampm}' rel='{$field['format']}' value='{$meta}' size='30' />";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_posts($field, $meta) {
            ...
            if ( 'checkbox_list' == $options['type']) {
                foreach ($posts as $p) {
                    echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-posts-checkbox" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='$p->ID'" . checked(in_array($p->ID, $meta), true, false) . " /> $p->post_title<br/>";
                }
            }
            // select
            else {
                echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-posts-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
        ...
        public function show_field_taxonomy($field, $meta) {
            ...
            if ( 'checkbox_list' == $options['type']) {
                foreach ($terms as $term) {
                    echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-tax-checkbox" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " /> $term->name<br/>";
                }
            }
            // select
            else {
                echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-tax-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
                foreach ($terms as $term) {
        ...
        public function show_field_dimension($field, $meta) {
            ...
            echo "<ul " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='online-magazine-dimension" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' id='{$field['id']}'>";
            if (in_array( 'left', $position)) {
                ...
                echo '<span class="ht-dimension-label">' . esc_html__( 'Left', 'online-magazine' ) . '</span>';
                echo '</li>';
            }
            ...
            if (in_array( 'top', $position)) {
                ...
                echo '<span class="ht-dimension-label">' . esc_html__( 'Top', 'online-magazine' ) . '</span>';
                echo '</li>';
            }
            ...
            if (in_array( 'bottom', $position)) {
                ...
                echo '<span class="ht-dimension-label">' . esc_html__( 'Bottom', 'online-magazine' ) . '</span>';
                echo '</li>';
            }
            ...
            if (in_array( 'right', $position)) {
                ...
                echo '<span class="ht-dimension-label">' . esc_html__( 'Right', 'online-magazine' ) . '</span>';
                echo '</li>';
            }

            echo '<li class="ht-dimension-wrap">';
            echo '<div class="ht-link-dimensions">';
            echo '<span class="dashicons dashicons-admin-links ht-linked" title="' . esc_html__( 'Link', 'online-magazine' ) . '"></span>';
            echo '<span class="dashicons dashicons-editor-unlink ht-unlinked" title="' . esc_html__( 'Unlink', 'online-magazine' ) . '"></span>';
            echo '</div>';
            echo '</li>';
            echo '</ul>';
            $this->show_field_end($field, $meta);
        ...
        public function addBackground($id, $args, $repeater = false) {
            $new_field = array( 'type' => 'background', 'id' => $id, 'desc' => '', 'name' => 'Background Field', 'std' => array( 'id' => '', 'url' => '', 'repeat' => 'no-repeat', 'size' => 'auto', 'position' => 'center center', 'attachment' => 'scroll', 'color' => '' ), 'multiple' => false);
            $new_field = array_merge($new_field, $args);
            if (false === $repeater) {
                $this->_fields[] = $new_field;
            } else {
        ...
        public function addGallery($id, $args, $repeater = false) {
            $new_field = array( 'type' => 'gallery', 'id' => $id, 'desc' => '', 'name' => 'Gallery Field', 'std' => '', 'multiple' => false);
            $new_field = array_merge($new_field, $args);
            if (false === $repeater) {
                $this->_fields[] = $new_field;
            } else {
