...
function update_repeater_fields() {
    online_magazine_metabox_fields.updateRepeater();
}
//metabox fields object
var online_magazine_metabox_fields = {
    ...
    fancySelect: function () {
        if ($().select2) {
            $(".online-magazine-select, .online-magazine-posts-select, .online-magazine-tax-select, .online-magazine-widget-select").each(function () {
    ...
    load_time_picker: function () {
        $( '.online-magazine-time' ).each(function () {
    ...
    load_date_picker: function () {
        $( '.online-magazine-date' ).each(function () {
    ...
    load_color_picker: function () {
        if ($( '.online-magazine-color-iris' ).length > 0)
            $( '.online-magazine-color-iris' ).wpColorPicker();
    },
    load_tab: function () {
...
window.setTimeout( 'online_magazine_metabox_fields.init();', 2000);

jQuery(document).ready(function ($) {
    // repater Field
    $("body").on( 'click', '.online-magazine-re-toggle', function () {
        $(this).closest( '.online-magazine-repater-block' ).find( '.ht-meta-box-repeater-table' ).slideToggle();
    });

    $( 'body' ).on( 'click', '.online-magazine-re-remove', function () {
        $(this).closest( '.online-magazine-repater-block' ).slideUp(500, function () {
            $(this).remove();
        });
    });
    ...
    $( '.repeater-sortable' ).sortable({
        opacity: 0.8,
        cursor: 'move',
        handle: '.online-magazine-re-sort-handle'
    });
    ...
    $(".viral-pro-linked").on("click", function () {
        ...
        var $this = $(this);
        // Remove linked class
        $this.parent().parent(".viral-pro-dimension-wrap").prevAll().slice(0, 4).find("input").removeClass("linked");
        // Remove class
        $this.parent(".viral-pro-link-dimensions").removeClass("unlinked");
    });
    // Unlinked button
    $(".viral-pro-unlinked").on("click", function () {
        ...
        var $this = $(this);
        // Add linked class
        $this.parent().parent(".viral-pro-dimension-wrap").prevAll().slice(0, 4).find("input").addClass("linked");
        // Add class
        $this.parent(".viral-pro-link-dimensions").addClass("unlinked");
    });
    // Values linked inputs
    $(".viral-pro-dimension-wrap").on("input", ".linked", function () {
